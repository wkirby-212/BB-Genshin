...
export default function CharacterDashboard({ onSelect, user }) {
  ...
  function handleJoinSubmit(e) {
    ...
    if (idx !== -1) {
      ...
      setCharacters(allChars); // Refresh UI to show new campaign link
    }
        ...
        <div key={char.id} style={{ padding: 18, background: "#f6fcf6", marginBottom: 10, borderRadius: 9 }}>
          ...
          {char.campaignIds && char.campaignIds.length > 0 && (() => {
            ...
            const joinedNames = char.campaignIds
              .map(cid => {
                const c = allCampaigns.find(camp => camp.id === cid);
      ...
      {showCreate && (
        <div style={{
          ...
          <form onSubmit={handleCreateCharacter} style={{
            ...
            <label>
              Name:<br />
              <input value={newName} onChange={e => setNewName(e.target.value)} autoFocus style={{ width: "100%", fontSize: 18, margin: "10px 0" }} />
            </label>
